...
if(isset($_POST['submit'])){
	...
	if((isset($_GET['GUID']) && $update) || (!isset($_GET['GUID']) && $insert)) {
		...
		if(isset($fts_tags) && $fts_tags!="" && $document_GUID!=""){
			...
			foreach($ftsTagsArr as $ftsTagVal){
				if($findExistingftsUUID=$db->get_var("select uuid from documents_fts where document_uuid='' and fts_content='$ftsTagVal'")){
					$db->query("update documents_fts set document_uuid='".$document_GUID."' where uuid='$findExistingftsUUID' and type='tags'");
				}else{
		...
		**/
		...
		if(isset($_POST['search_keywords']) && $_POST['search_keywords']!="" && $document_GUID!=""){
			$db->query("delete FROM documents_fts WHERE document_uuid='".$document_GUID."' and type='keywords'");
			
			$searchKeywords=$_POST['search_keywords'];
			$searchKeywordsArr=explode(",",$searchKeywords);
			foreach($searchKeywordsArr as $keyVal){
				$keyGUID = UniqueGuid('documents_fts', 'uuid');
				$db->query("INSERT INTO documents_fts(uuid, document_uuid, fts_content, type) VALUES('".$keyGUID."','$document_GUID','$keyVal','keywords')");
			}
		}elseif(isset($fts_tags) && $fts_tags=="" && $document_GUID!=""){
			$db->query("delete FROM documents_fts WHERE document_uuid='".$document_GUID."' and type='keywords'");
		}
...
$guid= $_GET['GUID'];
	$document = $db->get_row("SELECT ID, SiteID, AutoFormatMetaData,Document, Code, Body, Reference, Tags, FFAlpha80_3,FFAlpha80_2, FFAlpha80_4, FFReal01, FFReal02, FFAlpha80_7, FFAlpha80_1, Title,MetaTagKeywords, MetaTagDescription, UserID, PageTitle, Type, Published_timestamp, GUID, Sync_LastTime,AutoFormatTitle, PostedTimestamp, post_code, latitude, longitude, status, PublishCode, Template FROM documents where documents.GUID ='$guid'");
		...
		$search_keywords=""; 
		//$fts_tags="";
		$ftsTagQry= $db->get_results("SELECT * FROM documents_fts where document_uuid ='".$document->GUID."'");
		if(count($ftsTagQry)>0){
			foreach($ftsTagQry as $tag){
				...
				if($tag->type=="keywords"){
					if($search_keywords==""){
						$search_keywords.=$tag->fts_content;
					}else{
						$search_keywords.=",".$tag->fts_content;
					}
				}
	...
	<ul>
										...
										<div class="control-group">
											...
											<div class="control-group">
												...
												<div class="controls">
													...
													<input type="text" class="span6" name="reference" id="reference" value="<?php echo $Reference;?>" />
													<span class="help-block">&nbsp;</span>
				...
				<option <?php if($job_type == 'Part time') { echo "selected"; } ?> value='Part time' >Part time</option>
											...
											<div class="control-group" id="salary_range">
												...
												<div class="controls">
													...
													<div class="span4" style="width:25px; line-height:28px">&nbsp;To&nbsp;</div>
													<div class="span4" style="width:80px">
											...
											<div class="control-group">
												<label class="control-label">Search for location or postcode</label>
													<div class="span6" >
														<div class="ui-widget">
															<select onChange="" name="uk_towns_cities" id="uk_towns_cities" >
															<option value=""></option>	
																<?php	if($uk_towns=$db->get_results("select GUID, name, postcode from uk_towns_cities limit 30 ")){
																			foreach($uk_towns as $uk_town){ ?>
																			<option value="<?php echo $uk_town->GUID; ?>"><?php echo $uk_town->name; if(isset($uk_town->postcode) && $uk_town->postcode!=""){	echo' ('.$uk_town->postcode.')'; }	?></option>	
																		<?php }
																		}	?>
															</select>
														</div>
														<span class="help-block">Used to populate location. postcode, latitude and longitude below</span>
													</div>
													<div class="span3" style="float:right">
														<a href="location.php" class="btn btn-info" title="Add new UK Location" target="_blank">Add new UK Location</a>
													</div>
											</div>
											<div class="control-group">
												<label class="control-label">Location<span class="f_req">*</span></label>
													<div class="span4" style="width:130px">
														<input type="text" readonly class="span10" name="FFAlpha80_1" id="FFAlpha80_1" onKeyUp="generate_job_code('chk_manual')" onBlur="generate_job_code('chk_manual'); generate_metatags();" value="<?php echo $location;?>" style="width:200px;" />
														<span class="help-block">&nbsp;</span>
													</div>
													<label class="control-label">Post Code<span class="f_req">*</span></label>
													<div class="span4" style="width:130px">
														<input type="text" readonly class="span10" name="post_code" id="post_code" value="<?php echo $post_code;?>" style="width:130px;" />
														<span class="help-block">&nbsp;</span>
													</div>
											</div>
											<div class="control-group" id="lat_long_div">
												<label class="control-label">Latitude<span class="f_req">*</span></label>
													<div class="span4" style="width:130px">
													<input type="text" readonly class="span10" name="latt" id="latt" value="<?php echo $latitude;?>" style="width:130px;" />
													<span class="help-block">&nbsp;</span></div>
													<label class="control-label">Longitude<span class="f_req">*</span></label>
													<div class="span4" style="width:130px">
													<input type="text" readonly class="span10" name="long" id="long" value="<?php echo $longitude;?>" style="width:130px;" />
													<span class="help-block">&nbsp;</span>
													</div>
											</div>
											...
											<div class="control-group">
												...
												<div class="controls">
													<textarea type="text" class="span10 tagsClass" name="fts_tags" id="fts_tags" rows="1"></textarea>
													<span class="help-block">&nbsp;</span>
                                                </div>
											</div>
		...
		<option value="">-- Select User--</option>
								...
								<div class="input-append date" id="dp2">
									<input class="input-small" placeholder="Posted Date" type="text" readonly  name="Published_timestamp" id="Published_timestamp" value="<?php echo $date; ?>"  data-date-format="dd/mm/yyyy" /><span class="add-on"><i class="splashy-calendar_day"></i></span>
								</div>
								
								<div>
									<span class="help-block">&nbsp;</span>
									<input type="text" class="span8" id="tp_2" name="pub_time" value="<?php echo $time_string; ?>" readonly placeholder="Published Time" />
								<span class="help-block">&nbsp;</span>
								</div>
									...
									<div class="form-horizontal well">
										<fieldset>
											...
											<div class="control-group">
												...
												<div class="controls">
													<input type="text" class="span10" name="Code" id="Code" <?php if($auto_format!=0) { ?> readonly <?php } ?> value="<?php echo $Code;?>" />
												...
												<div class="controls">
													...
													<input type="text" class="span10" name="WindowTitle" id="WindowTitle" <?php if($AutoFormatMetaData!=0) { ?> readonly <?php } ?> value="<?php echo $WindowTitle;?>" />
													<span class="help-block">&nbsp;</span>
											...
											<div class="control-group">
												...
												<div class="controls">
													<textarea cols="30" rows="5" name="MetaTagKeywords" id="MetaTagKeywords" class="span10" <?php if($AutoFormatMetaData!=0) { ?> readonly <?php } ?>><?php echo $MetaTagKeywords;?></textarea>
												...
												<div class="controls">
													<textarea cols="30" rows="5" name="MetaTagDescription" id="MetaTagDescription" class="span10" <?php if($AutoFormatMetaData!=0) { ?> readonly <?php } ?>><?php echo $MetaTagDescription;?></textarea>
											...
											<div class="control-group" <?php if(isset($_GET['show_search_tags']) && ($_GET['show_search_tags']==true || $_GET['show_search_tags']=="true")){	}else{ echo 'style="display:none;"'; } ?>>
												<label class="control-label">Site search keywords</label>
												<div class="controls sKeywordsClass">
													<textarea type="text" class="span10 tagsClass" readonly="true" name="search_keywords" id="search_keywords" rows="1"></textarea>
													<span class="help-block">&nbsp;</span>
                                                </div>
											</div>
										</fieldset>
...
<?php echo $select_cats; ?>
			...
			function generate_metatags(){
				...
				var metaTagsDisplayNum=0;
				if(status!=true){
					metaTagsDisplayNum=1;
				}
					...
					$.ajax({                
						...
						data: {searchDisplayNum: 1, metaTagsDisplayNum: metaTagsDisplayNum, site_id : site, document_name: document_name, body :body, subtitle : $('#PageTitle').val(), post_code : $('#post_code').val(), category1 : $('#category1').val(), category2: $('#category2').val() , category3:$('#category3').val() , category4:$('#category4').val() ,category5:$('#category5').val() , category6:$('#category6').val() , location:location },
						cache: false,
						dataType: "json",  
						success:  function(response){ 
							if(response){
								...
								if(response.SearchKeywords!=''){
									var keyTagsStr=response.SearchKeywords;
  									var keywordsTagsArrStr =  new Array();
  									//if(keyTagsStr!=""){
  										keywordsTagsArrStr = keyTagsStr.split(',');
  									//}
  									$('#search_keywords').tagEditor('destroy');
  									$('#search_keywords').tagEditor({
   						 				initialTags: keywordsTagsArrStr,
    									delimiter: ',',
			...
			function fetchuktownsdetails(){
				$("#locErrMsg").remove();
				var uuid=$( "#uk_towns_cities" ).val();
				if(uuid!=""){
					$.ajax({
						type: "POST",
						url: "fetchuktowndetail.php",
						data: {'uuid' : uuid},
						cache: false,
						dataType: "json",  
						success: function(response){
							...
							if(response.name){
								$('#FFAlpha80_1').val(response.name);
							}else{
								missingArr.push("Location");
								$('#FFAlpha80_1').val('');
							}
							if(response.postcode){
								$('#post_code').val(response.postcode);
							}else{
								missingArr.push("Postcode");
								$('#post_code').val('');
							}
							if(response.lat){
								$('#latt').val(response.lat);
							}else{
								missingArr.push("Latitude");
								$('#latt').val('');
							}
							if(response.lng){
								$('#long').val(response.lng);
							}else{
								missingArr.push("Longitude");
								$('#long').val('');
							}
							if($('#FFAlpha80_1').val()!=""){
							...
							if(missingArr.length>=1){
								var missingPara=missingArr.toString(); 
								var errMsg="<div class='span12' id='locErrMsg' style='color:#CC0000;font-style:italic;font-size: 11px;padding-bottom:10px;'><label class='control-label'>&nbsp;</label><b>Note:</b> "+missingPara+" are missing in database, please update these fields. <a href='location.php?GUID="+uuid+"' target='_blank' class='btn btn-info' style='font-style: normal'>Update location</a></div>";
								$("#lat_long_div").after(errMsg);
							}
							}
						}
					});
			...
			var val_flag=0;
				$(document).ready(function() {
					...
					$('.tagsClass').keypress(function(event){
						if(event.keyCode == 13) {
  					...
  					var ftsTagsArrStr=new Array();
  					//if(ftsTagsArrStr!=""){
  						ftsTagsArrStr = ftsTagsStr.split(',');
  					//}
  					var existingTagsArr="";
...
<?php } ?>
tiny_options['setup'] = function(editor) {  editor.on('blur', function (e) {  generate_metatags();  });  editor.on('init', function (e) {  generate_metatags();  }); };
tinymce.init(tiny_options);
