...
require_once("lib.inc.php");
	 ...
	 */
	$aColumns = array('documents.GUID', 'documents.ID','sites.Name', 'documents.Reference', 'documents.Document','documents.PageTitle', "CONCAT( wi_users.firstname, ' ', wi_users.lastname ) as UserName", 'documents.Modified','documents.PostedTimestamp','documents.Status', 'sites.WebsiteAddress', 'documents.Code','documents.Body');
	
	$aColumns1 = array('GUID', 'WebsiteAddress', 'Code', 'ID', 'Name', 'Reference', 'Document', 'PageTitle','UserName', 'Modified', 'PostedTimestamp', 'Status');
		...
		for ( $i=0 ; $i<count($aColumns1) ; $i++ )
		{
			...
			elseif ( $aColumns1[$i] == "GUID" ){
				...
				if($aTotal[0]==0){
					...
					$jobApplications = '<a  href="jobapps.php?job_guid='.$curr_id.'" class="page_code" target="_blank">'.$aTotal[0].'</a>';
				}
			}
