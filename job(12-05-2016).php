<?php 
// Tags in fts field so search work as full text search
...
if(isset($_POST['submit'])){
	...
	if((isset($_GET['GUID']) && $update) || (!isset($_GET['GUID']) && $insert)) {
		...
		if(isset($fts_tags) && $fts_tags!="" && $document_GUID!=""){
			if($findExistingftsUUID=$db->get_var("select uuid from documents_fts where document_uuid='' and fts_content='$fts_tags'")){
				$db->query("update documents_fts set document_uuid='".$document_GUID."' where uuid='$findExistingftsUUID'");
				//$db->debug();
			}elseif($findExistingftsUUID=$db->get_var("select uuid from documents_fts where document_uuid='$document_GUID'")){
				$db->query("update documents_fts set fts_content='".$fts_tags."' where uuid='$findExistingftsUUID' and document_uuid='$document_GUID'");
				//$db->debug();
			}else{
				$ftsGUID = UniqueGuid('documents_fts', 'uuid');
				$db->query("INSERT INTO documents_fts(uuid, document_uuid, fts_content) VALUES('".$ftsGUID."','$document_GUID','$fts_tags')");
				//$db->debug();
			}
		}
...
$guid= $_GET['GUID'];
	$document = $db->get_row("SELECT ID, SiteID, AutoFormatMetaData,Document, Code, Body, Reference, Tags, FFAlpha80_3,FFAlpha80_2, FFAlpha80_4, FFReal01, FFReal02, FFAlpha80_7, FFAlpha80_1, Title,MetaTagKeywords, MetaTagDescription, UserID, PageTitle, Type, Published_timestamp, GUID, Sync_LastTime,AutoFormatTitle, PostedTimestamp, post_code, latitude, longitude, status, PublishCode, Template FROM documents where documents.GUID ='$guid'");
		...
		if($ftsTagQry= $db->get_row("SELECT * FROM documents_fts where document_uuid ='".$document->GUID."' ")){
			$fts_tags=$ftsTagQry->fts_content;
		}else{
			$fts_tags='';
		}
